...
DEFAULT_MAXIMUM_WORD_LENGTH: int = 100
NUMBER_OF_TEST_CASES: int = 10

MINIMUM_OPERATIONS_PER_TEST_CASE: int = 10000
MAXIMUM_OPERATIONS_PER_TEST_CASE: int = 800000
...
def generate_test_case(number_of_operations: int):
    ...
    for _ in range(number_of_operations):
        ...
        # Test case: remove(word) [no output]
            ...
            if string in pseudo_trie:
                i: int = stored_words.index(string)
...
def main():
    ...
    to_do: int = MAXIMUM_OPERATIONS_PER_TEST_CASE
    ...
    for tc in range(NUMBER_OF_TEST_CASES):
        ...
        # Ensure the remaining test cases can still satisfy the min/max limits.
        min_allowed = max(
            MINIMUM_OPERATIONS_PER_TEST_CASE,
            to_do - tests_left * MAXIMUM_OPERATIONS_PER_TEST_CASE,
        )
        max_allowed = min(
            MAXIMUM_OPERATIONS_PER_TEST_CASE,
            to_do - tests_left * MINIMUM_OPERATIONS_PER_TEST_CASE,
        )

        if min_allowed > max_allowed:
            raise ValueError(
                "Impossible constraints: adjust MINIMUM_OPERATIONS_PER_TEST_CASE, "
                "MAXIMUM_OPERATIONS_PER_TEST_CASE, NUMBER_OF_TEST_CASES, or "
                "MAXIMUM_OPERATIONS_PER_TEST_CASE."
            )

        operations: int = random.randint(min_allowed, max_allowed)
        to_do -= operations
